...
local function ignore_case_eq(l_v, r_v)
    if not l_v then
        return false
    end
    if not r_v then
    ...
    if str_lower(l_v) == str_lower(r_v) then
        return true
    end
    return false
end

local function ignore_case_in(l_v, r_v)
    if not l_v then
        return false
    end
    if not r_v then
    ...
    if type(r_v) == "table" then
        for _, v in ipairs(r_v) do
            if str_lower(v) == str_lower(l_v) then
                return true
            end
        end
    end
...
local compare_funcs = {
    ...
    ["has"] = has_element,
    ["ipmatch"] = ip_match,
    ["ignore_case_eq"] = ignore_case_eq,
    ["ignore_case_in"] = ignore_case_in,
}
...
local function deal (ctx, l_v)
    if not l_v then
        return nil
    end
    if core_str.has_suffix(str_lower(l_v), "appid") then
        if core_str.has_prefix(l_v, "post_arg_") then
            local appid = ctx['post_arg_appid']
            if appid then
                return appid
            end
            appid = ctx['post_arg_appId']
            if appid then
                return appid
            end
            appid = ctx['post_arg_APPID']
        ...
        if core_str.has_prefix(l_v, "arg_") then
            local appid = ctx['arg_appid']
            if appid then
                return appid
            end
            appid = ctx['arg_appId']
        ...
        if core_str.has_prefix(l_v, "http_") then
            local appid = ctx['http_appid']
            if appid then
                return appid
            end
            appid = ctx['http_appId']
            if appid then
                return appid
            end
            appid = ctx['http_APPID']
