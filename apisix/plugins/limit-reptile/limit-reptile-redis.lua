...
function _M.incoming(self, key)
    ...
    core.log.info("redis 连接开始：host:" .. conf.redis_host .. ",port:" .. conf.redis_port)
    local ok, err = red:connect(conf.redis_host, conf.redis_port or 6379)
    if not ok then
        core.log.error("redis 连接失败！")
        return false, err
    end
    ...
    key = conf.application .. "::" ..conf.env .. ":" .. key
    core.log.info("redis连接工具，生成key：", key,",limit：",limit,",window:",window)
    remaining, err = red:eval(script, 1, key, limit, window)
    core.log.info("redis连接工具，执行结果：", remaining, "，错误信息：", err)
    if err then
        return nil, err
    end
