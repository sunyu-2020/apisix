...
function _M.rewrite(conf, ctx)
    ...
    if conf.sample_ratio == 1 or math.random() < conf.sample_ratio then
        ...
        local correlation = {}
        if ctx.var.group_version then
            correlation["groupVersion"] = ctx.var.group_version
        end
        if ctx.var.http_apisix_user_id then
            correlation["userId"] = ctx.var.http_apisix_user_id
        end

        local real_ip = ctx.var.realip_remote_addr or ctx.var.remote_addr or ''

        if real_ip then
            correlation["remoteIp"] = real_ip
        end

        sw_tracer:start("upstream service", correlation)
