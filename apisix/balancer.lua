...
local room = require("apisix.room")
...
local lrucache_room_server_picker = core.lrucache.new({
    ttl = 300, count = 256
})
local lrucache_other_room_server_picker = core.lrucache.new({
    ttl = 300, count = 256
})
...
local function room_fetch_health_nodes(upstream, checker, isTheRoom)

    local nodes = upstream.nodes

    -- 多机房处理
    nodes = room.get_room_nodes(nodes, isTheRoom)
    if not checker then
        local new_nodes = core.table.new(0, #nodes)
        for _, node in ipairs(nodes) do
            new_nodes = transform_node(new_nodes, node)
...
local function do_create_server_picker(upstream, up_nodes)
    ...
    if picker then
        ...
        server_picker.addr_to_domain = addr_to_domain
        core.log.info("server_picker:", core.json.delay_encode(server_picker, true))
...
local function create_other_server_picker(upstream, checker)

    local up_nodes = room_fetch_health_nodes(upstream, checker, false)
    if core.table.nkeys(up_nodes) == 0 then
        return nil
    end
    return do_create_server_picker(upstream, up_nodes)
end

local function create_room_server_picker(upstream, checker)

    local up_nodes = room_fetch_health_nodes(upstream, checker, true)
...
local function parse_addr(addr)
    local host, port, room, err = core.utils.parse_addr(addr)
    return { host = host, port = port, room = room }, err
end
...
local function get_other_server_picker(ctx, from_room_picker)
    local up_conf = ctx.upstream_conf

    local version = ctx.upstream_version
    local key = ctx.upstream_key
    local checker = ctx.up_checker
    local other_room_server_picker = ctx.other_room_server_picker
    -- 多机房
    if not other_room_server_picker then
        other_room_server_picker = lrucache_other_room_server_picker(key .. "other_room", version,
                create_other_server_picker, up_conf, checker)
    end

    if not other_room_server_picker then
        return nil, "failed to fetch other room server picker"
    end
    -- 从机房选择器转发过来的，需要触发下一次轮询
    if from_room_picker then
        other_room_server_picker.after_balance(ctx, true)
    end

    local other_room_server, err = other_room_server_picker.get(ctx)

    if not other_room_server then
        err = err or "no valid upstream node"
        return nil, "failed to find valid other room upstream server, " .. err
    end

    ctx.balancer_server = other_room_server
    local domain = other_room_server_picker.addr_to_domain[other_room_server]
    local res, err = lrucache_addr(other_room_server, nil, parse_addr, other_room_server)
    if err then
        core.log.error("failed to parse room server addr: ", other_room_server, " err: ", err)
        return core.response.exit(502)
    end
    res.domain = domain
    ctx.other_room_server_picker = other_room_server_picker
    return res
end

local function get_room_server_picker(ctx)
    local up_conf = ctx.upstream_conf
    local version = ctx.upstream_version
    local key = ctx.upstream_key
    local checker = ctx.up_checker
    ...
    local resFin
    local room_server_picker = ctx.room_server_picker
    -- 多机房
    if not room_server_picker then
        room_server_picker = lrucache_room_server_picker(key .. "room", version,
                create_room_server_picker, up_conf, checker)
    end
    if not room_server_picker then
        return nil, "failed to fetch room server picker"
    end

    local room_server, _ = room_server_picker.get(ctx)

    if not room_server then
        -- ctx.room_server_picker = nil
        return nil, "failed to fetch room server picker"
    end
    ctx.balancer_server = room_server

    local domain = room_server_picker.addr_to_domain[room_server]
    local res, err = lrucache_addr(room_server, nil, parse_addr, room_server)
    if err then
        core.log.error("failed to parse room server addr: ", room_server, " err: ", err)
        return core.response.exit(502)
    end
    resFin = res
    resFin.domain = domain
    ctx.room_server_picker = room_server_picker
    return resFin
end
...
local function pick_server(route, ctx)
    ...
    local version = ctx.upstream_version
    local checker = ctx.up_checker
    local has_other_picker
    ctx.balancer_try_count = (ctx.balancer_try_count or 0) + 1
    if ctx.balancer_try_count > 1 then
        if ctx.room_server_picker and ctx.room_server_picker.after_balance then
            ctx.room_server_picker.after_balance(ctx, true)
        end

        if ctx.other_room_server_picker and ctx.other_room_server_picker.after_balance then
            has_other_picker = true
            ctx.other_room_server_picker.after_balance(ctx, true)
        end
    ...
    local res, err
    if not has_other_picker then
        res, err = get_room_server_picker(ctx)
    end
    if not res then
        res, err = get_other_server_picker(ctx, ctx.room_server_picker)
    end
