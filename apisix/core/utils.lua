...
local function room_parse_ipv4_or_host(addr)
    local room_key = rfind_char(addr, "@", #addr - 1)
    if not room_key then
        local ip, port = _parse_ipv4_or_host(addr)

        return ip, port, nil
    end

    local room = sub_str(addr, 1, room_key - 1)

    local port_host = sub_str(addr, room_key + 1)

    local host, port = _parse_ipv4_or_host(port_host)
...
function _M.parse_room_addr(addr)
    if str_byte(addr, 1) == str_byte("[") then
        -- IPv6 format, with brackets, maybe with port
        local right_bracket = str_byte("]")
        local len = #addr
        ...
        for i = 1, #addr do
            local ch = str_byte(addr, i, i)
            if ch == dot then
                return room_parse_ipv4_or_host(addr)
            elseif ch == colon then
                colon_counter = colon_counter + 1
                if colon_counter == 2 then
                    return _parse_ipv6_without_port(addr)
