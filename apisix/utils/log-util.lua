...
local function latency_details_in_ms(ctx)
    ...
    ctx.var.gateway_time = apisix_latency
...
local function get_yzw_log(ngx, conf)
    local ctx = ngx.ctx.api_ctx
    local var = ctx.var
    local service_id
    local route_id
    ...
    local log = {
        ...
        uri = var.request_uri,
        method = ngx.req.get_method(),
        request_headers = core.json.encode(ngx.req.get_headers()),
        request_size = var.request_length,
        status = ngx.status,
        scheme = ctx.var.scheme,
        host = ctx.var.host,
        response_headers = core.json.encode(ngx.resp.get_headers()),
        response_size = var.bytes_sent,
        server = core.utils.gethostname() .. ':' .. core.version.VERSION,
        upstream = var.upstream_addr,
        service_id = service_id,
        route_id = route_id,
        consumer = consumer,
        client_ip = core.request.get_ip(ngx.ctx.api_ctx),
        start_time = ngx.req.start_time() * 1000,
        latency = latency,
        upstream_latency = upstream_latency,
        apisix_latency = apisix_latency
    ...
    local sw8 = ngx.var.http_sw8
    if sw8 ~= nil then
        local ref = SegmentRef.fromSW8Value(sw8)
        log.trace_id = ref.trace_id
    end
...
function _M.get_log_entry(plugin_name, conf, ctx)
    ...
    if conf.log_format or has_meta_log_format then
        ...
        -- entry["detail"] = core.json.encode(get_full_log(ngx, conf))
    else

        if is_http then
            if conf.mode == "yzw" then
                entry = get_yzw_log(ngx, conf)
            else
