...
local upstream_path = require("apisix.path")
local error = error
...
function _M.set_by_route(route, api_ctx)
    ...
    -- 上游类型-全路径
    local path_type = up_conf.path_type
    if path_type then
        upstream_path.rewrite(api_ctx)
    end
    -- 上游类型：服务发现
...
local function filter_upstream(value, parent)
    ...
    if core.table.isarray(nodes) then
        ...
        for addr, weight in pairs(nodes) do
            local host, port, room = core.utils.parse_room_addr(addr)
            if not core.utils.parse_ipv4(host) and
                    not core.utils.parse_ipv6(host) then
                parent.has_domain = true
            end
